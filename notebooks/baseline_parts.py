# Within this file is parallelization over params = (k_fold, C)
# This could be modified to parallelize at the level of jobs, 
# taking k_fold and C as config parameters
...
# If config['n_parallel'] is None, set it to the number of jobs
# if config['n_parallel'] is None:
#     config['n_parallel'] = config['k_fold'] * len(config['C_list'])

write_dir = '/data/MLDSST/xinaw/impactme/results/raw/baseline_parts'
...
def train(params):
    ...
    if j == -1:
        ...
        df = fit_lr(
            embeds_np[outer_trn], y[outer_trn], 
        ...
        df = fit_lr(
            embeds_np[inner_trn], y[inner_trn], 
...
# Parallelize over params = (k_fold, C)
params = list(
    itertools.product(
...
# print(f'Parallelizing over {len(params)} jobs')
...
with open(f'{write_dir}/{config["model_f"]}-{config["data"]}-{config["feature"]}-{config["C_part"]}.pkl', 'wb') as f:
    pickle.dump(lr, f)
